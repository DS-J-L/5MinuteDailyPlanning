  ...
  return (
    <article className="glass-panel rounded-[28px] border border-white/70 p-5 shadow-panel">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <p className="text-xs uppercase tracking-[0.2em] text-pine/70">기록</p>
          <h3 className="mt-2 text-2xl font-semibold text-ink">{formatLongDate(item.date)}</h3>
          <p className="mt-2 text-sm text-ink/70">
            {completedCount}/3 완료 · {formatMinutes(totalMinutes)}
          </p>
        </div>
        <span className="rounded-full bg-accentSoft px-4 py-2 text-sm font-medium text-ink">최신순</span>
      </div>
      ...
      <div className="mt-5 rounded-[24px] bg-pine px-4 py-4 text-white">
        <p className="text-xs uppercase tracking-[0.2em] text-white/70">회고</p>
        <p className="mt-2 text-sm leading-6">{item.review || "아직 작성된 회고가 없습니다."}</p>
      </div>
    </article>
  );
}
