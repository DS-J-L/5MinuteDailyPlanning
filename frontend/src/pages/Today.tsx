    ...
    <div className="space-y-6">
      <section className="grid gap-6 lg:grid-cols-[1fr_0.75fr]">
        <div className="glass-panel rounded-[32px] border border-white/70 p-6 shadow-panel">
          <p className="text-sm uppercase tracking-[0.2em] text-pine/70">Today plan</p>
          <h2 className="mt-2 text-3xl font-semibold text-ink">Keep the day intentionally small.</h2>
          <p className="mt-3 text-sm text-ink/70">
            {plan ? `${completedCount}/3 complete · ${formatMinutes(totalMinutes)}` : "No plan has been saved yet for today."}
          </p>
        </div>

        <div className="glass-panel rounded-[32px] border border-white/70 p-6 shadow-panel">
          <p className="text-sm uppercase tracking-[0.2em] text-pine/70">Next step</p>
          <h3 className="mt-2 text-2xl font-semibold text-pine">Close the loop with a short review.</h3>
      ...
      {plan?.tasks.length ? (
        ...
        <div className="glass-panel rounded-[28px] border border-dashed border-ink/15 p-6 text-center text-sm text-ink/60">
          Save a daily plan first to see your Big 3 here.
        </div>
      )}
